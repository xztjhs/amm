...
```bash
# 克隆仓库
git clone <YOUR_GIT_REPO_URL> /amm
cd /amm
...
打开浏览器访问: `http://<服务器IP>:<WEB_PORT>/`

> 统一 Web 入口（容器内 80，宿主映射 <WEB_PORT>）：
> - `http://<host>:<WEB_PORT>/` 直接打开 AMM WebUI 管理页面
> - `http://<host>:<WEB_PORT>/api/*` 管理/推理接口
> - `http://<host>:<WEB_PORT>/v1/*` OpenAI 兼容 API（chat/embedding/asr/tts/ocr/images/videos）
>
> OpenAI 客户端 base_url 填：`http://<服务器IP>:<WEB_PORT>/v1`
...
> 端口映射（`docker-compose.yml`）：`<WEB_PORT>->80`(Web/API 统一入口)、`60007->443`(HTTPS 预留)、`62220->22`(SSH)。
