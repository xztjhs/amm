...
> **平台**: <YOUR_SERVER_IP> 容器（Unraid 宿主）
> **SSH**: `ssh -p <SSH_PORT> root@<YOUR_SERVER_IP>`（密码 <YOUR_PASSWORD>）
...
> **Web UI**: `http://<YOUR_SERVER_IP>:<WEB_PORT>/`
>
> **统一访问端口（2026-08-07 起）**: 容器仅监听 80，宿主映射 <WEB_PORT>，单入口同时承载
> - WebUI 页面：`http://<host>:<WEB_PORT>/`
> - 管理/推理 API：`http://<host>:<WEB_PORT>/api/*`
> - OpenAI 兼容 API：`http://<host>:<WEB_PORT>/v1/*`（base_url 填 `http://<host>:<WEB_PORT>/v1`）
> - 旧 8080/<API_PORT>、18080 端口迁移自本文历史；不再对外服务。
...
- [x] **vLLM 0.22.1（CUDA 13 + Blackwell 验证通过）**：
  ...
  - chat 实例已切 vllm/0.22.1 + `/models/Qwen3-4B`（safetensors），外部 <API_PORT> 全链路推理验证通过
...
- [x] AMM server 运行中（统一端口 80，宿主映射 **<WEB_PORT>**；WebUI + /api/* + /v1/* 同一入口）
