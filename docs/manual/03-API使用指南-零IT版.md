...
### 需要什么前提？
- 能访问 AMM 服务器（浏览器能打开 `http://<服务器>:<WEB_PORT>`）
...
> 小提醒：把命令里的 `<服务器>` 换成 AMM 的 IP（如 `<YOUR_SERVER_IP>`）。
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/chat/completions \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/embeddings \
  -H "Content-Type: application/json" \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/audio/speech \
  -H "Content-Type: application/json" \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/audio/transcriptions \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/rerank \
  -H "Content-Type: application/json" \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/images/generations \
  -H "Content-Type: application/json" \
...
```bash
curl http://<服务器>:<WEB_PORT>/v1/videos/generations \
  -H "Content-Type: application/json" \
