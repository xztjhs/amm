...
from backend.engines.quantizer import setup_routes as setup_quantize_routes
...
def create_app() -> web.Application:
    ...
    setup_quantize_routes(app, manager)
...
async def main():
    ...
    # 统一监听端口 (容器内 80, 宿主机映射 <WEB_PORT>):
    #   同时承载 WebUI 页面(/、/api管理、/static) 与 OpenAI 兼容 API(/v1/*)
    #   前端与外部客户端均通过 http://host:<WEB_PORT> 访问 (相对路径同源)
    port = config.get("server", {}).get("port", 80)

    app = create_app()
    logger.info(f"AMM 启动: http://{host}:{port}  (WebUI + OpenAI 兼容 API, 宿主机映射 <WEB_PORT>)")

    runner = web.AppRunner(app, handle_signals=False)
    await runner.setup()
    site = web.TCPSite(runner, host, port)
    await site.start()
    logger.info(f"[AMM] 已监听 0.0.0.0:{port} (宿主机映射 <WEB_PORT>): WebUI + OpenAI API /v1/* + 管理 /api/*")
