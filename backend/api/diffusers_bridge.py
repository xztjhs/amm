...
# 强制 diffusers / transformers 离线加载: AMM 内所有模型均走本地 /models 磁盘,
# 不触碰 huggingface.co (内网不可达, 否则 from_pretrained 会挂起等待网络超时)
...
def _write_model_log(model_id: str, line: str) -> None:
    ...
    try:
        ...
        _safe_id = str(model_id).replace("/", "_").replace("\\", "_")
        with open(os.path.join(_LOGS_DIR, f"{_safe_id}_server.log"), "a", encoding="utf-8") as f:
            f.write(f"[{ts}] {line}\n")
...
# 全局 pipeline 加载互斥锁 (2026-08-08 修复: 防止 t2v/i2v 并发重复加载
# 同一个 Wan2.2-A14B 大模型, 避免 CPU 满核 + 内存/GPU资源挤占导致"假死")
_PIPE_LOAD_LOCK = asyncio.Lock()


async def _load_pipeline(model_cfg: Dict) -> Any:
    """异步加载 diffusers pipeline (串行锁 + 二次检查, 防并发重复加载)

    在 _load_pipeline_impl 之上加 .全局串行锁: 同一时刻只允许一个协程真正执行
    权重加载(from_pretrained 阻塞, 会占大量 CPU/内存); 其余并发请求在拿到锁后
    二次命中缓存直接返回, 避免重复加载同一个大模型。
    """
    ...
    async with _PIPE_LOAD_LOCK:
        # 二次检查: 等待锁期间可能有其他请求已加载完成
        if cache_key in _pipeline_cache:
            return _pipeline_cache[cache_key]
        return await _load_pipeline_impl(model_cfg, model_id, category, model_source, cache_key)


async def _load_pipeline_impl(model_cfg: Dict, model_id: str, category: str, model_source: str, cache_key: str) -> Any:
    ...
    category = model_cfg.get("category", "")
    model_source = model_cfg.get("model_source", "modelscope")
    # 用短实例 id (t2v/t2i/i2v) 作为日志名, 与页面 get /instances/{id}/logs 对齐
    log_id = model_cfg.get("id") or model_id

    _model_log(log_id, f"开始加载 pipeline (source={model_source}, quant={model_cfg.get('quant','default')})")
    ...
    def _load():
        ...
        _model_log(log_id, f"[load] CUDA 可用={_cuda_ok}", extra={"device_name": torch.cuda.get_device_name(0) if _cuda_ok else None})

        local = _resolve_local_path(model_id)
        logger.info(f"Local model path: {local}")
        _model_log(log_id, f"[load] 开始读取权重 (读取到内存, 尚未搬运 GPU)")
        ...
        _model_log(log_id, f"[load] 权重读取完成 (从磁盘读入内存)")

        # ---- FP8 layerwise_casting ----
        if _should_enable_layerwise_casting(model_cfg):
            _model_log(log_id, f"[load] 应用 FP8 layerwise_casting 量化 (存储 fp8, 计算 bf16)...")
            _apply_layerwise_casting(pipe)
            _model_log(log_id, f"[load] FP8 量化完成")

        # ---- 显存搬运策略 (2026-08-08 显式) ----
        offload = _resolve_offload(model_cfg)
        _model_log(log_id, f"offload 策略 = {offload}")
        if offload == "group":
            _model_log(log_id, f"[load] 应用 leaf_level group_offload (部分权重驻 CPU)...")
            _apply_group_offload(pipe, offload_to_cpu=True)
            try:
                if torch.cuda.is_available() and getattr(pipe, "vae", None) is not None:
                    pipe.vae.to("cuda")
                    _model_log(log_id, f"[load] VAE 已搬运到 GPU")
            except Exception as e:
                logger.warning(f"VAE 移回 GPU 失败: {e}")
        elif offload == "model":
            try:
                _model_log(log_id, f"[load] 应用 enable_model_cpu_offload (序列级 CPU offload)...")
                pipe.enable_model_cpu_offload()
                logger.info("enable_model_cpu_offload enabled (model offload)")
                _model_log(log_id, f"[load] model offload 已启用")
            except Exception as e:
                logger.warning(f"enable_model_cpu_offload 失败: {e}")
                if torch.cuda.is_available():
                    pipe = pipe.to("cuda")
                    _model_log(log_id, f"[load] 回退: 全模型搬运 GPU")
        else:  # gpu - 全驻 GPU
            if torch.cuda.is_available():
                _model_log(log_id, f"[load] 全模型搬运到 GPU (pipe.to cuda)...")
                pipe = pipe.to("cuda")
                logger.info(f"全模型驻留 GPU (to cuda), offload={offload}")
                try:
                    alloc = round(torch.cuda.memory_allocated()/1024**2,1)
                    _model_log(log_id, f"[load] 已完成驻留 GPU", extra={"cuda_allocated_mb": alloc})
                except Exception:
                    _model_log(log_id, f"[load] 已完成驻留 GPU")
            else:
                logger.warning("CUDA 不可用, diffusers 回退 CPU 推理")
                _model_log(log_id, f"[load] CUDA 不可用, 回退 CPU 推理 (仅限验证使用)")
    ...
    try:
        ...
        _model_log(log_id, f"Pipeline 加载失败: {e}")
        raise
    _pipeline_cache[cache_key] = pipe
    _model_log(log_id, "Pipeline 加载完成")
