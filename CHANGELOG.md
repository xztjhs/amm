...
1. **HTTP 代理设置**：
   ...
   - 开启后下载（HF/ModelScope）经代理转发；测试代理 `<YOUR_PROXY_URL>`。
...
### ✅ 遗留问题修复
- **统一访问端口：WebUI + OpenAI 兼容 API 均走 80 → <WEB_PORT>**
  - 单一端口承载三者：`/`(WebUI) + `/api/*`(管理/推理) + `/v1/*`(OpenAI 兼容)
  - Web 访问：`http://<host>:<WEB_PORT>/`；API base_url：`http://<host>:<WEB_PORT>/v1`
  - 容器内仅剩一个监听端口 80，移除废弃的 8080/<API_PORT> 映射
- **Wan2.2-T2V 冷启动验证通过** 🎉
